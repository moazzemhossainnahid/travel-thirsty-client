    ...
    return (
        <div className='w-full mx-auto rounded'>
            ...
            <form onSubmit={handleSubmit(handleSigninform)} action="" className='py-3'>
                <input {...register('email', { required: { value: true, message: 'Email is Required' }, pattern: { value: /^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/, message: 'Provide a Valid Email' } })} required type="email" placeholder="Enter Your Email" className="input bg-slate-100 my-2 block mx-auto input-ghost w-full max-w-xs" />
                <label htmlFor="email">{errors?.email?.type === 'required' && <span className='text-red-700'>{errors.email?.message}</span>}</label>
                <label htmlFor="email">{errors?.email?.type === 'pattern' && <span className='text-red-700'>{errors.email?.message}</span>}</label>
